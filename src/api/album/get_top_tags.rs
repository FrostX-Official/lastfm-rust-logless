use crate::{
    api::{LastfmMethod, ParameterBuilder}, APIResponse, AlbumGetTopTagsResponse, Error, Lastfm, Result
};
use reqwest::Method;
...
impl<'a> AlbumGetTopTags<'a> {
    ...
    pub async fn send(self) -> Result<APIResponse<AlbumGetTopTagsResponse>> {
        self.validate()?;
